...
# HologramID
    execute store result score @e[tag=HologramChar,distance=..0.0001,limit=1] HologramID run data get storage hologram_ui: TextGroup[-1].HologramID
# resolve
...
# Name
    data modify entity @e[tag=HologramChar,distance=..0.0001,limit=1] CustomName set from entity @e[type=item,distance=..0,y=300,limit=1] Item.tag.display.Lore[-1]
# resolveItem kill
